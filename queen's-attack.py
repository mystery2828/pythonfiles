...
def attack(queen_pos, obs_pos, dimension,matrix):
    ...
    #computing row_left
    for i in range(queen_pos[1]-1,-1,-1):
    ...
    #computing row_right
    for j in range(queen_pos[1]+1,dimension):
        if matrix[queen_pos[0]][j] is not 1:
            n=n+1
        else:
            break
    
    #computing column_up
    for k in range(queen_pos[0]-1,-1,-1):
        if matrix[k][queen_pos[1]] is not 1:
            n+=1
        else:
            break

    #computing column_down
    for l in range(queen_pos[0]+1,dimension):
        if matrix[l][queen_pos[1]] is not 1:
            n+=1
        else:
            break
